...
class ExpectFunctions {
  constructor(actual) {
    this.actual = actual;
    this.stringifiedAutual = JSON.stringify(actual);
  }

  toExist() {
    if (this.actual == null )
    throw `expected value to exist but got ${this.stringifiedAutual}`;
  }

  toBe(expected) {
    if (this.actual !== expected) {
      throw `expected ${this.stringifiedAutual} to be ${JSON.stringify(expected)}`
    }
  }
